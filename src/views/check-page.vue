<template>
    <content-layout title="Verifica locatia">
        <div class="alert-error" style="display: none">{{ this.alertMessage }}</div>
        <ion-card color="light">

            <ion-card-header>
                <ion-item>
                    <ion-label v-if="this.valoareOfInput_loc.length > 0" position="floating">Scrie VTE - {{
                            this.valoareOfInput
                    }}</ion-label>
                    <ion-label v-else position="floating">Cauta locatia</ion-label>
            ...
            if (this.currentLocation != '' && this.currentLocation != 'null') {
                ...
                if (this.valoareOfInput.startsWith('LOC ')) {
                    if (this.sendVTEForAdd == false) {
                        ...
                        await db.query("SELECT * FROM `barcode_match` WHERE `Locatie` = '" + this.valoareOfInput_loc + "' AND `Matchpoint` = '" + this.currentLocation + "';")
                            .then((e) => {
                        ...
                        let db_statement = "INSERT INTO `barcode_match_logs` (`Logs`) VALUES ('- [ERROR]: Something went wrong (sendVTEForAdd) - check page');";
                    ...
                    if (this.valoareOfInput_loc.length > 0 || this.sendVTEForAdd) {
                        ...
                        let db_statement = "INSERT INTO `barcode_match_logs` (`Logs`) VALUES ('- [ERROR]: Something went wrong (sendVTEForAdd || valOfInput_loc invalid) - check page');";
                        await db.query(db_statement)
                            .then(() => { console.log("ok inserted log"); })
                            .catch(e => {
                                alert(e);
                ...
                this.showWarninigError(3000, 'WARNING: VTE-ul invalid (nu contine VTE-).');
