    ...
    <content-layout title="Barcode">
        <ion-card color="light">
            <ion-card-header>
                ...
                <table v-if="this.valoareSQL.length > 0 && this.stare == 'cauta'"
                ...
                <table v-if="this.valoareSQL.length > 0 && this.stare == 'complet'"
                    style="font-family:Arial;width:100%;border:1px solid #ccc;border-collapse: collapse;color:#667677;">
                    <thead
                        style="border:1px solid #ccc;background:#6a9ef2;color:white !important; height:50px;padding:2px;">
                        <tr style="border:1px solid #ccc;">
                    ...
                    <tbody>
                        <tr style="border:1px solid #ccc;" v-for="barcode in valoareSQL" :key="barcode">
                            ...
                            <td v-if="this.d3.length > 0" style="border:1px solid #ccc;text-align:center;padding:3px;"
                                class="string">{{ barcode.locThree }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table v-if="this.valoareSQL.length > 0 && this.stare == 'verifica'"
                    style="font-family:Arial;width:100%;border:1px solid #ccc;border-collapse: collapse;color:#667677;">
                    <thead
                        style="border:1px solid #ccc;background:#6a9ef2;color:white !important; height:50px;padding:2px;">
                        <tr style="border:1px solid #ccc;">
...
export default defineComponent({
    ...
    components: { IonCard, IonCardHeader, IonButton, IonCardContent, QrcodeStream },
    methods: {
        async decode(codeQrScanned) {
            ...
            if (this.currentLocation != '' && this.currentLocation != 'null') {
                if (this.stare == 'cauta') {
                    if (this.codeQr.startsWith("VTE-")) {
                        if (this.stare != 'adauga_vte_in_locatie') {
                            ...
                            if (this.currentLocation == 'm') {
                                ...
                                sql_query = "SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d1 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p1 + "' " + this.locatie_material + " order by TimeStamp desc LIMIT 1 ) as tableA";
                                sql_query += " UNION ALL SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d2 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p2 + "' " + this.locatie_material + " order by TimeStamp desc LIMIT 1 ) as tableB;";
                                ...
                                sql_query = "SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d1 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p1 + "' " + this.locatie_material + " order by TimeStamp desc LIMIT 1 ) as tableA ";
                                sql_query += "UNION ALL SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d2 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p2 + "' " + this.locatie_material + " order by TimeStamp desc LIMIT 1 ) as tableB ";
                                sql_query += "UNION ALL SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d3 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p3 + "' " + this.locatie_material + " order by TimeStamp desc LIMIT 1 ) as tableC;";
                            } else {
                                this.p1 = 'C'; // c
                                this.d1 = 'PIEPTI';
                                ...
                                sql_query = "SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d1 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p1 + "' order by TimeStamp desc LIMIT 1 ) as tableA";
                                sql_query += " UNION ALL SELECT * FROM ( SELECT [TimeStamp] ,[Order] ,REPLACE([Locatie], 'LOC ', '') as [Locatie] ,'" + this.d2 + "' as Element FROM barcode_match where [Order] = '" + this.codeQr.toUpperCase() + "' and Element = '" + this.p2 + "' order by TimeStamp desc LIMIT 1 ) as tableB;";
                    ...
                    if (this.codeQr.startsWith('VTE-')) {
                        await db.query("INSERT INTO `barcode_match` (`Order`, `Locatie`, `Element`, `Matchpoint`) VALUES ('" + this.codeQr + "', '" + this.valLocatie + "', '" + this.valLocatie.split(' ').pop().split('-')[0] + "', '" + this.currentLocation + "');")
                            .then(() => {
                    ...
                    if (this.codeQr.toUpperCase().startsWith('LOC ')) {
                        ...
                        await db.query("SELECT * FROM `barcode_match` WHERE `Locatie` = '" + this.codeQr.toUpperCase() + "' AND `Matchpoint` = '" + this.currentLocation + "';")
                            .then((e) => {
                            ...
                            await db.query("SELECT * FROM `barcode_match` WHERE `Locatie` = '" + this.codeQr.toUpperCase() + "' AND `Matchpoint` = '" + this.currentLocation + "';")
                                .then((e) => {
                                    Object.entries(e).forEach(([key, value]) => {
                                        ...
                                        this.output = "Scaneaza comanda pentru [" + this.valoareOfInput_loc + "]";
                                    });
                                })
                                .catch((e) => {
                                    alert(e);
        ...
        async globalForLoop() {
            ...
            this.valoareSQL = [];
            // porneste camera ca sa poti scana.
        },
        buttonCauta() {
            this.stare = 'cauta';
